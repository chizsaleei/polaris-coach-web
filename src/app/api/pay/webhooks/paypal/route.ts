// src/app/api/pay/webhooks/paypal/route.ts
...
const CORE_PATH = "/api/pay/webhooks/paypal";

/**
 * PayPal webhook entrypoint for the web app.
 * Forwards raw headers + body to Polaris Core, which:
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.error("paypal_webhook_read_body_failed", { error: String(err) });
  ...
  try {
    ...
    logger.error("paypal_webhook_core_unreachable", { error: String(err) });
    return NextResponse.json(
      {
        ok: false,
...
function requireEnv(name: string) {
  ...
  if (!value) {
    throw new Error(`Missing environment variable ${name} for PayPal webhook route`);
  }
  return value;
}
