// polaris-coach-web/src/app/(app)/account/delete/page.tsx
...
function errorCopy(code: string) {
  if (code === "confirm") return "You must type DELETE exactly to confirm.";
  if (code === "pending") return "A deletion job is already queued for your account.";
  return "We could not reach the delete service. Try again or contact support.";
}
...
function DeletionStatusCard({ job }: { job: DeleteJob | null }) {
  ...
  const requestedOn = job?.created_at ? formatDate(job.created_at) : null;
  const scheduledFor = job?.scheduled_at ? formatDate(job.scheduled_at) : null;
  const completedOn = job?.finished_at ? formatDate(job.finished_at) : null;
  ...
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-900/5 dark:bg-[#03111A]">
      ...
      <dl className="mt-6 grid gap-4 text-sm text-slate-600 dark:text-slate-300 sm:grid-cols-2">
        <InfoRow label="Requested on" value={requestedOn ?? (hasJob ? "Unknown" : "Not requested")} />
        <InfoRow label="Scheduled for" value={scheduledFor ?? (hasJob ? "Not scheduled" : "Not scheduled")} />
        <InfoRow label="Completed" value={completedOn ?? (job ? "Pending" : "-")} />
        <InfoRow label="Reference" value={job?.id ?? "-"} />
      </dl>
...
async function scheduleDeletion(formData: FormData) {
  ...
  const scheduleAt =
    delayDays > 0 ? new Date(Date.now() + delayDays * 86_400_000).toISOString() : undefined;
